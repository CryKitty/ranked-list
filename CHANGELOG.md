...
- Tightened non-colon title splitting so sequel-marker-plus-subtitle names like `Resident Evil 4 Remake` and `Final Fantasy VII Rebirth` can split after the number, while one-token suffixes like `Silent Hill f` stay as full titles.
...
- Debounced board search filtering by two seconds so the board stops re-filtering on every keystroke.
- Shared links now initialize their modal defaults from the live board view/filter state, and the shared Tier List view better matches the owner styling while allowing full vertical scrolling on mobile.
...
- Fixed a tier-list drag crash regression by moving the active insert-target state sync out of collision detection and back into the normal drag-move path.
